...
// Кэш-запись с записями и временем истечения
struct CacheEntry {
    records: Vec<Record>,
    expires_at: Instant,
}

// Кэш в виде потокобезопасного DashMap
...
async fn main() -> Result<()> {
    ...
    // Фоновая задача очистки кеша и префетчинга
...
fn recursive_lookup_with_cache(
    ...
    Box::pin(async move {
        ...
        loop {
            ...
            // Отправляем запросы параллельно всем серверам
            ...
            // Обработка делегирования (referral)
...
/// Отправляет UDP DNS-запрос и ожидает ответ с таймаутом
...
/// Вспомогательная функция для извлечения IP из RData
